...
class Request:
	...
	def pack(self) -> bytes:
		base = pack("!BBBB", self.version, self.command.value, self.reserved, self.address_type.value)
	...
	def unpack(cls, stream: BytesIO | socket) -> "Request":
		if hasattr(stream, 'read'):
			...
			reader = stream.recv
		version, command_raw, reserved, address_type_raw = unpack_from("!BBBB", reader(4))
		command, address_type = Command(command_raw), AddressType(address_type_raw)
		dest_addr = b""
	...
	@classmethod
	def create(cls, command: Command, address_type: AddressType, dest_addr: bytes, port: int) -> "Request":
...
class Reply:
	...
	def pack(self) -> bytes:
		base = pack("!BBB", self.version, self.reply.value, self.address_type.value)
		if self.address_type == AddressType.DOMAIN_NAME:
			base += pack("!B", len(self.dest_addr))
		base += self.dest_addr + pack("!H", self.port)
		return base
	...
	def unpack(cls, stream: BytesIO | socket) -> "Reply":
		...
		reply_status, address_type = ReplyStatus(reply_raw), AddressType(address_type_raw)
		dest_addr = b""
		if address_type == AddressType.IP_V4:
			dest_addr = reader(4)
		elif address_type == AddressType.IP_V6:
	...
	@classmethod
	def create(cls, reply_status: ReplyStatus, address_type: AddressType, dest_addr: bytes, port: int) -> "Reply":
		return cls(VERSION, reply_status, address_type, dest_addr, port)
