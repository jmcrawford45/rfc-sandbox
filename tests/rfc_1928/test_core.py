...
def test_basic_auth_request_serde():
	...
	assert BasicAuthRequest.unpack(BytesIO(BasicAuthRequest.pack(start))) == start
...
Request.create(Command.UDP_ASSOCIATE, AddressType.DOMAIN_NAME, b"0" * 8, 0x1234),
])
def test_request_serde(item):
	assert Request.unpack(BytesIO(Request.pack(item))) == item

@pytest.mark.parametrize("item", [
Reply.create(ReplyStatus.SUCCEEDED, AddressType.IP_V4, b"0" * 4, 0x1234),
Reply.create(ReplyStatus.NETWORK_UNREACHABLE, AddressType.IP_V6, b"0" * 16, 0x1234),
Reply.create(ReplyStatus.CONNECTION_REFUSED, AddressType.DOMAIN_NAME, b"0" * 8, 0x1234),
])
